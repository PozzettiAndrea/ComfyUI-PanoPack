...
def process_single_scene(scene_dir, scene_name, mesh, args, segmentation_data=None,
    ...
    if global_mesh_origin_down_distance is not None and np.isfinite(global_mesh_origin_down_distance):
        ...
        # Raycast missed everything (0/8 hits) — most often a hole in the
        # mesh directly under the panorama capture point (where MoGe / the
        # depth-merge has worst coverage). Fall back to the mesh's AABB to
        # estimate floor distance, then clamp to the same indoor/outdoor
        # range the raycast result would have been clamped to. Only fall
        # back to args.agentHeight if even the AABB is degenerate (no
        # vertices, or origin is below the mesh).
        try:
            mesh_v = np.asarray(mesh.vertices)
            if mesh_v.size > 0:
                # Mesh hasn't been rotated to Y-up yet (rotation at line
                # ~3176); Z is up in this original frame.
                z_min = float(mesh_v[:, 2].min())
                z_floor_distance = float(-z_min)  # distance below origin
                if not np.isfinite(z_floor_distance) or z_floor_distance <= 0:
                    raise ValueError(f"non-positive AABB floor distance: {z_floor_distance}")
                if is_outdoor:
                    z_floor_distance = float(np.clip(z_floor_distance, 0.2, 0.75))
                else:
                    z_floor_distance = float(np.clip(z_floor_distance, 0.28, 0.78))
                rank0_log(f"[{scene_name}] AABB-fallback floor distance: {z_floor_distance:.3f} "
                          f"(z_min={z_min:.3f}, raycast was 0/8 hits)")
                agentHeight = z_floor_distance * 1.2
                agentRadius = agentHeight * 0.4
                agentMaxClimb = agentHeight * 0.5
            else:
                raise ValueError("mesh has no vertices")
        except Exception as _aabb_err:
            rank0_log(f"[{scene_name}] Global mesh origin down distance is None or not finite, "
                      f"AABB fallback also failed ({_aabb_err}), using args defaults.")
            agentHeight = args.agentHeight
            agentRadius = args.agentRadius
            agentMaxClimb = args.agentMaxClimb

    # ---- DIAG: floor detection + agent dims ----
    try:
        hit_ratio = (int(np.sum(valid_hit_mask)), int(len(valid_hit_mask))) \
            if 'valid_hit_mask' in dir() else (None, None)
        print(f"[DIAG process_single_scene] scene={scene_name} is_outdoor={is_outdoor} "
              f"floor_down_dist={global_mesh_origin_down_distance} "
              f"floor_hit_ratio={hit_ratio[0]}/{hit_ratio[1]} "
              f"agentHeight={agentHeight:.3f} agentRadius={agentRadius:.3f} "
              f"agentMaxClimb={agentMaxClimb:.3f}", flush=True)
    except Exception as _e:
        print(f"[DIAG process_single_scene] (failed to print agent dims: {_e})", flush=True)

    with timer.track("Get navmesh and post-processing"):

        # 1. Rotation (Z-Up -> Y-Up for Recast)

        # ---- DIAG: mesh bounds BEFORE rotation (Z-up original frame) ----
        try:
            _v_before = np.asarray(mesh.vertices)
            print(f"[DIAG process_single_scene] mesh BEFORE rotation (Z-up): "
                  f"verts={len(_v_before)} "
                  f"min={_v_before.min(0).tolist()} max={_v_before.max(0).tolist()}",
                  flush=True)
        except Exception as _e:
            print(f"[DIAG process_single_scene] (mesh-before print failed: {_e})", flush=True)

        R_to_yup = mesh.get_rotation_matrix_from_xyz((-np.pi / 2, 0, 0))
        mesh.rotate(R_to_yup, center=(0, 0, 0))

        # ---- DIAG: mesh bounds AFTER rotation (Y-up Recast frame) ----
        try:
            _v_after = np.asarray(mesh.vertices)
            print(f"[DIAG process_single_scene] mesh AFTER rotation (Y-up Recast): "
                  f"min={_v_after.min(0).tolist()} max={_v_after.max(0).tolist()}",
                  flush=True)
        except Exception as _e:
            print(f"[DIAG process_single_scene] (mesh-after print failed: {_e})", flush=True)
        ...
        if pv_flat is None:
            ...
            # ---- DIAG: navmesh sample verts after all post-processing
            #            (still in Y-up Recast frame) ----
            try:
                _nv = np.asarray(nav_verts, dtype=np.float32)
                if len(_nv):
                    n_show = min(3, len(_nv))
                    sample_idxs = [0, len(_nv) // 2, len(_nv) - 1][:n_show]
                    print(f"[DIAG process_single_scene] navmesh: {len(_nv)} verts "
                          f"(Y-up). Y-range=[{_nv[:,1].min():.3f}, {_nv[:,1].max():.3f}] "
                          f"X-range=[{_nv[:,0].min():.3f}, {_nv[:,0].max():.3f}] "
                          f"Z-range=[{_nv[:,2].min():.3f}, {_nv[:,2].max():.3f}]", flush=True)
                    for i in sample_idxs:
                        print(f"[DIAG process_single_scene]   nav_verts[{i}] = "
                              f"{_nv[i].tolist()}", flush=True)
                else:
                    print(f"[DIAG process_single_scene] navmesh empty (0 verts)", flush=True)
            except Exception as _e:
                print(f"[DIAG process_single_scene] (navmesh sample print failed: {_e})", flush=True)

...
# WorldNav: replace vLLM/OpenAI client with HF transformers Qwen3-VL inline.
# `openai` is still lazy-imported inside `get_traj_caption` for users who
# point that function at a vLLM server URL, but the primary path goes
# through `QwenVLClient` below.

from .general_utils import load_video


# ----------------------------------------------------------------------
# QwenVLClient — OpenAI-shaped wrapper around HF transformers Qwen3-VL.
# ----------------------------------------------------------------------
#
# Preserves the `client.chat.completions.create(model=..., messages=...,
# max_tokens=..., temperature=..., seed=...)` shape used by upstream
# traj_generate.py and the three call sites in this file, so the existing
# prompts port verbatim.
#
# Input message format mirrors OpenAI's:
#   [{"role": "system", "content": "..."},
#    {"role": "user",   "content": [
#        {"type": "text", "text": "..."},
#        {"type": "image_url", "image_url": {"url": "data:image/png;base64,..."}}]}]
#
# Internally we map each `image_url` entry to a PIL image, build Qwen3-VL's
# chat template, run `model.generate(...)`, and wrap the decoded text in a
# small `.choices[0].message.content` namespace so callers don't care
# whether they're talking to OpenAI, vLLM, or our HF wrapper.
...
def _decode_data_url(url: str) -> Image.Image:
    """Decode a `data:image/...;base64,...` URL to a PIL image."""
    m = _DATA_URL_RE.match(url)
    if not m:
        raise ValueError(f"unsupported image url; expected data:image/...;base64,..., got {url[:80]}")
    return Image.open(io.BytesIO(base64.b64decode(m.group(1)))).convert("RGB")


def _convert_openai_messages_to_qwen(
    messages: List[dict],
) -> tuple[List[dict], List[Image.Image]]:
    """Convert OpenAI-shaped messages into Qwen3-VL chat-template format.

    OpenAI:   {"type": "image_url", "image_url": {"url": "data:image/...;base64,..."}}
    Qwen3-VL: {"type": "image"}   (the PIL image is passed alongside via `images=`)

    Returns (qwen_messages, images_list).
    """
    ...
    for msg in messages:
        ...
        for part in content:
            ...
            elif t == "image":  # already in Qwen format
                images.append(part["image"])
                new_parts.append({"type": "image"})
            else:
                # Pass through unknown types (e.g. "video"); Qwen will error if unsupported.
                new_parts.append(part)
        qwen_messages.append({"role": role, "content": new_parts})
    return qwen_messages, images


class _ChatCompletions:
    def __init__(self, client: "QwenVLClient"):
        self._client = client
    ...
    def create(
        ...
        **_ignored,
        ...
        return self._client._generate(messages, max_tokens, temperature, seed)
...
class QwenVLClient:
    """HF-transformers replacement for the OpenAI/vLLM client.

    Construct once per process; `.chat.completions.create(...)` mirrors the
    OpenAI API surface that upstream code expects.

    Args:
        model_id: HF repo id; defaults to Qwen3-VL-2B-Instruct.
        device:   torch device (`"cuda"`, `"cpu"`, or torch.device).
        dtype:    torch dtype; bf16 on CUDA, fp32 on CPU.
        attn_impl: pass `"flash_attention_2"` to use flash-attn if available;
                  defaults to `"sdpa"` which works without extra deps.
    """
    ...
    def _generate(
        ...
        import torch  # local
        qwen_messages, images = _convert_openai_messages_to_qwen(messages)
        ...
        decoded = self.processor.batch_decode(new_ids, skip_special_tokens=True)[0]

        return _ChatResponse(choices=[_Choice(message=_Message(content=decoded))])
...
def get_traj_caption(LLM_ADDR, LLM_PORT, MODEL_NAME, traj_path, sample_count=8):
    from openai import OpenAI  # lazy — see top of file

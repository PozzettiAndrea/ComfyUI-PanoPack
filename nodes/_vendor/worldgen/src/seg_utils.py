...
# `zim_anything` lazy-imported inside build_zim_model/DetPredictor for users
# who still want the ZIM path. WorldNav's primary segmentation is SAM3
# (`transformers.Sam3Model`) which traj_generate.py loads directly.
...
# build zim model (optional path — not used by WorldNav's SAM3-only flow)
def build_zim_model(ZIM_MODEL_CONFIG, ZIM_CHECKPOINT, device="cuda"):
    from zim_anything import zim_model_registry  # lazy
...
# Lazy ZimPredictor base: zim_anything isn't installed in WorldNav's env;
# the DetPredictor subclass below is only constructed when build_zim_model
# is called, which never happens on WorldNav's SAM3-only trajectory path.
try:
    from zim_anything import ZimPredictor as _ZimPredictorBase
except ImportError:
    class _ZimPredictorBase:  # placeholder so the class definition below loads
        def __init__(self, *args, **kwargs):
            raise NotImplementedError(
                "zim_anything is not installed. WorldNav uses SAM3 for "
                "segmentation; install zim_anything only if you re-enable the "
                "ZIM path."
            )


class DetPredictor(_ZimPredictorBase):
    def predict(
            self,
